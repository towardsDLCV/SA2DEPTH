...
from networks.SADEDepth import SADEDepth

from datetime import datetime
from sum_depth import Sum_depth


parser = argparse.ArgumentParser(description='SADE PyTorch implementation.', fromfile_prefix_chars='@')
parser.convert_arg_line_to_args = convert_arg_line_to_args

parser.add_argument('--mode',                      type=str,   help='train or test', default='train')
parser.add_argument('--model_name',                type=str,   help='model name', default='sade')
parser.add_argument('--encoder',                   type=str,   help='type of encoder, small and large', default='large')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # model
    model = SADEDepth(encoder=args.encoder, max_depth=args.max_depth, min_depth=args.min_depth,
                      pretrained=None)
    model.train()
...
def main():
    ...
    if save_files:
        ...
        command = 'cp sade/train.py ' + aux_out_path
        os.system(command)
        command = 'mkdir -p ' + networks_savepath + ' && cp sade/networks/*.py ' + networks_savepath
        os.system(command)
        command = 'mkdir -p ' + dataloaders_savepath + ' && cp sade/dataloaders/*.py ' + dataloaders_savepath
        os.system(command)
